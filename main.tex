...
\begin{definition}\label{def:complex-characteristic-class} A \emph{complex characteristic class} is a natural transformation
\begin{align*}
    \Vect_\C^k(-) \Rightarrow H^r(-,\Z)
\end{align*}
of functors $\Top^\op \to \Set$ (or perhaps from $\Ho(\Top)^\op \to \Set$).
\end{definition}

When we restrict our attention to paracompact spaces, we have that $\Vect_\C^k(-)$ is \emph{representable}, when viewed as a functor out of $\Ho(\Top)$. So a characteristic class is a natural transformation of the form
\begin{align*}
    \left[ -,\Gr_\C(k,\infty) \right] \Rightarrow H^r(-,\Z).
\end{align*}
%
But by the Yoneda lemma, the set of natural transformations of the above is in bijection with the cohomology class $H^r(\Gr_\C(k,\infty))$. So we get the following:

\begin{slogan} A complex characteristic class for rank $k$ vector bundles is an integral cohomology class of the complex Grassmannian $\Gr_\C(k,\infty)$.
\end{slogan}

\begin{theorem}[Borel]
We have an isomorphism
\begin{align*}
    H^\ast(\Gr_\C(r,\infty);\Z) \cong \Z[c_1, \ldots, c_n],
\end{align*}
where $c_i$ lives in degree $i$.
\end{theorem}
...
\end{align*}
We construct $c_k(E)$ to be the pullback of $c_k \in H^{2k}(\Gr_\C(r,\infty),\Z)$ under the map $f$.
...
\begin{align*}
    g_{ij} g_{jk} g_{ki} = 1,
\end{align*}
and we define $c_1$ to be
\begin{align*}
    c_1 := f_{ij} + f_{jk} + f_{ki} \in \Z.
\end{align*}
We claim this integer doesn't depend on the choice of triple overlap $i,j,k$, and that this integer is in fact the first Chern class of the bundle.
