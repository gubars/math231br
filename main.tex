...
Thank you to Zo\"{e} Batterman, Vincent Costa, Frank Gubars, Grayson Kemplin, Gabe Ong, and Greg Wong for offering suggestions and changes to the notes. Thanks to Gage Martin and Sidhanth Raman for helpful conversations in preparing this class.
...
\begin{itemize}
    \item the refinement map $\check{H}^0(\mathcal{U},F) \to \check{H}^1(\mathcal{V},F)$ is an isomorphism
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        U &\mapsto \colim_{\substack{V\in \Open(Y) \\ V \supseteq f(U)}} G(U).
    \end{align*}
...
\begin{enumerate}
    ...
    \item we have
    \begin{align*}
        \alpha \wedge \beta = - \beta \wedge \gamma
    \end{align*}
    \item and
    \begin{align*}
        \alpha \wedge \alpha = 0.
...
\begin{example} On $M = \R^3$, an element of $\Gamma(\R^3,\Omega^1_\dR)$ is expressible as
\begin{align*}
...
\begin{example} Again on $M = \R^3$, an element of $\Gamma(\R^3,\Omega^2_\dR)$ is something of the form
\begin{align*}
...
Let's now consider the scaling map
\begin{align*}
    h \colon B \times [0,1] &\to B \\
    (x,t) &\mapsto tx.
\end{align*}
We can compute the pullback of $\omega$ along $h$ as
\begin{align*}
    h^\ast(\omega) &= f(tx_1, \ldots, tx_n) d(tx_{i_1})\wedge \cdots \wedge d(tx_{i_k}) \\
    % &= f(tx_1, \ldots, tx_n) \left( dx_I  + \sum_{j=1}^n dx_{i_1} \wedge \cdots \wedge dx_{i_{j-1}} \wedge dt \wedge dx_{i_{j+1}}\wedge \cdots \wedge dx_{i_k}\right) \\
    &= f(tx_1, \ldots, tx_n) \left( dx_I + dt\wedge \sum_{j=1}^n (-1)^j dx_{I\minus\{j\}} \right).
\end{align*}
We're going to leverage this to construct a primitive for $\omega$, namely we will define $\alpha$ by ``integrating out'' $t$ from $h^\ast \omega$. Explicitly:
\begin{align*}
    \alpha &= \left(\int_{0}^1 f(tx_1, \ldots, tx_n)dt\right) \sum_{j=1}^n (-1)^j dx_{I\minus\{j\}}.
\end{align*}
We claim that $d\alpha = \omega$. Note that when we differentiate, it is with respect to the $x_i$'s so we can pull it inside the integral. We get:
\begin{align*}
    d\alpha &= d \left(  \left(\int_{0}^1 f(tx_1, \ldots, tx_n)dt\right) \sum_{j=1}^n (-1)^j dx_{I\minus\{j\}}. \right) \\
    &=  \sum_{s=1}^n\left( \int_0^1 \frac{\partial f}{\partial dx_{s}}(tx_1, \ldots, tx_n) dt \right) dx_s \wedge \left( \sum_{j=1}^n (-1)^j dx_{I\minus\{j\}} \right)
\end{align*}
As all the differentials vanish unless $s\in I$, we get the above is equal to
\begin{align*}
    \sum_{j=1}^k \left(\int_0^1 \frac{\partial f}{\partial x_{i_j}}dt\right) dx_I.
\end{align*}
Finally to verify this equals $\omega$ we use \Cref{exer:fund-thm-calc}. This is a standard version of the fundamental theorem of calculus that holds in star-shaped domains; see for instance \cite[I.1.4]{tuIntroductionManifolds2011}).
\end{proof}

In general, as we know, applying $\Gamma(M,-)$ to the above sequence will turn it into a chain complex, but will fail to preserve exactness. Here's an easy case where it does preserve exactness: I claim we know this exactness from multivariable calculus!
...
\end{equation}
\end{proposition}
\begin{proof} Identifying $\Omega^1$ and $\Omega^2$ with vector fields as above, these differentials take a more familiar shape:
\begin{align*}
...
Our isomorphism in question is exhibited by integration. At the level of presheaves, given an open neighborhood $U \subseteq M$ and an element $\omega \in \Omega^k_\dR(U)$, we obtain a \v{C}ech cochain by integration along simplices in $U$:
\begin{align*}
...
so that
\begin{align*}
    (h_i h_j^{-1})(u,f) = u\cdot g_{ij}(u)f
\end{align*}
...
Given a topological group $G$, we can define the sheaf of groups $G_c$ as in \Cref{nota:Gc}, and therefore construct its \v{C}ech cohomology following \Cref{def:Cech-cohomology-on-cover}. Note that in the previous definition we had assumed our sheaf to be a sheaf of abelian groups, so we have to be a bit careful about products versus sums.
...
\begin{aligned}
     h_U' \circ k \circ h_U^{-1}(u,f) = u \times g_U(u)f
\end{aligned}
\end{equation}
for all $u,f$.
\end{definition}
...
\begin{terminology} A \emph{local basis} of a space $X$ at a point $x$ is any final subcategory of $\Open_x(X)$.
\end{terminology}
